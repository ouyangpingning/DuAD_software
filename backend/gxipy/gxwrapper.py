...
if sys.platform == 'linux2' or sys.platform == 'linux':
    ...
    if os.path.isdir(_local_libs):
        ...
        # 再按依赖序显式加载（logutil → gxiapi → dxmediaproc → dxvideoproc）
        cur = os.environ.get("LD_LIBRARY_PATH", "")
        os.environ["LD_LIBRARY_PATH"] = _local_libs + ((":" + cur) if cur else "")
        ...
        _load("libdxvideoproc.so", [os.path.join(_local_libs, "libdxvideoproc.so")])
    else:
        try:
...
class GxFrameData(Structure):
    _fields_ = [
        ...
        ('frame_id', c_ulonglong),              # The frame id of the image
        ('timestamp', c_ulonglong),             # Time stamp of image
        ('user_param', c_ulonglong),            # User param
        ('reserved',  c_int),                   # Reserved

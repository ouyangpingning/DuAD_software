...
- `backend/` — 后端（已从 pyqt5 复制并适配 PySide6）：
  ...
  - `libs/` — SDK 动态库（libgxiapi + ffmpeg 全家桶 + **GxU3VTL.cti 等 GenICam 传输层文件**，从官方 Galaxy_camera.run 提取，**无需 root 安装**）。⚠️ **.cti 必须齐全**：缺传输层时 gx_init_lib 返回 -1、枚举永远 0 台（即使相机在 lsusb 可见、udev 权限正确）；udev 规则在 `backend/config/99-galaxy-dev.rules`（需 root 装到 /etc/udev/rules.d/，否则普通用户无 USB 权限同样枚举不到）。⚠️ **x86 版 GxU3VTL 传输层对 >~1.7MB 单帧负载拒绝启动**（`ACQUISITION_START` 返回 -1010 "TL Error: Unable to start acquisition"，2448×2048 全幅在此 SDK 上无法采集，1224×1024 正常）——这是该版本 x86 SDK 的**传输层限制，非应用 bug**；全幅采集请用板子（arm64 SDK 2.4.2507.8231 正常，实测 79fps）或更换更新的 x86 SDK。`CameraBridge.startGather` 失败时会自动「重注册采集回调重建流缓冲」重试一次并记录原始错误码（`camera.py::_last_error_str`）。

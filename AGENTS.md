...
- `backend/` — 后端（已从 pyqt5 复制并适配 PySide6）：
  ...
  - `libs/` — SDK 动态库（libgxiapi + ffmpeg 全家桶 + **GxU3VTL.cti 等 GenICam 传输层文件**，从官方 Galaxy_camera.run 提取，**无需 root 安装**）。⚠️ **.cti 必须齐全**：缺传输层时 gx_init_lib 返回 -1、枚举永远 0 台（即使相机在 lsusb 可见、udev 权限正确）；udev 规则在 `backend/config/99-galaxy-dev.rules`（需 root 装到 /etc/udev/rules.d/，否则普通用户无 USB 权限同样枚举不到）。⚠️ **x86 版 GxU3VTL 传输层对 >~1.74MB 单帧负载拒绝启动**（`ACQUISITION_START` 返回 -1010 "TL Error: Unable to start acquisition"，2448×2048 全幅在此 SDK 上无法采集，1224×1024 正常；与帧率无关、重连/拔插无效）——这是**上游 x86 Linux U3VTL 的限制**（已验证**最新官方 SDK 2.6.2606 / lib 2.0.2604.8241 同样存在**，非仓库旧版问题，不存在可通过换新 SDK 修复的路径；arm64 2.4.2507 与 Windows 均无此问题）。全幅采集请用板子或 Windows。`CameraBridge.startGather` 对 x86+超大负载直接给出明确中文提示（不无意义重试）；其余 -1010 失败会自动「重注册采集回调重建流缓冲」重试一次并记录原始错误码（`camera.py::_last_error`）。
  - `Src/onnx_infer.py` — 联调用的无 torch 推理模块（原 `backend/alg/deploy/onnx_infer.py`，已随 `alg/` 删除迁至 `Src/`）：加载 pca_inline 模式 onnx，输入 numpy RGB 帧 → 返回 jet 热力图 RGB + 分数；预处理（Resize+CenterCrop(target_size)+ImageNet 归一化）在客户端完成；**后处理（上采样到 target_size + 高斯平滑）已内化进新 ONNX 图**（输出 heatmaps=[B,1,target,target] 即平滑后 amap，`predict` 直接取 `heatmaps[0,0]`，不再重复 numpy 高斯模糊；旧模型输出 patch 级 [B,H,W]，向后兼容仍走 `_gaussian_blur`，Pillow 12 的 GaussianBlur 不支持 float32 图）；**分数为判别器负输出的 patch max——带符号、无界、未归一化**（正常图 ≈ -1~1.5、异常图 > 2），与标定阈值比较判异常（**阈值来源优先级：ONNX metadata（训练时标定写入 ckpt→export 写 metadata，见训练仓库 `thresh_utils`）> `*.threshold.json` > 默认 1.7**），**不要** clamp/min-max 到 0~1（会把任何图归一成恒 1 误报）；target_size 默认 518，须与导出时一致

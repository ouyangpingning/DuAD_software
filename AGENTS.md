...
- **Jetson 上 TRT 依 JetPack 版本而定**：`onnx_infer.py` 在 aarch64 默认优先 `CUDAExecutionProvider`（x86 仍 TRT 优先；`DUAD_PREFER_TRT=1` 覆盖）。
  ...
  - **JetPack 7.2+（TRT 10.13+）**：TRT 数值**正确**（zip 实测 缺陷/好图 与 CUDA 分数差 ~0.001、无哨兵，TRT ~140~190ms/帧）。开 `DUAD_PREFER_TRT=1` 即可用（`run_jetson.sh` 已默认设）。**fp16 也已实测可用**（`DUAD_TRT_FP16=1`，`run_jetson.sh` 已默认开）：zipper 模型 ~190→~92ms（2×）、分数偏差 <0.02；引擎缓存走 `~/.cache/duad_trt_engine_fp16`（ORT 缓存名不含 fp16 标记，必须分目录）。
